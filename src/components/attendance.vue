      ...
      <v-simple-table :height="ih" :fixed-header="true">
        <thead>
          <tr>
            <th colspan="2" class="pt-2">
              <v-text-field label="Badge (Search)" v-model="search.badge" outlined dense />
            </th>
            <th class="pt-2">
              <v-text-field label="Name (Search)" v-model="search.name" outlined dense />
            </th>
            <th class="pt-2">
              <v-text-field label="Events (Search)" v-model="search.events" outlined dense />
            </th>
            <th width="1">
        ...
        <template v-for="(reg, i) in displayReg">
          <tr :key="i" class="veteran" v-if="reg.veteran.events.length">
            <td colspan="2">
              {{ reg.veteran.badgeName || `${reg.veteran.firstName} ${reg.veteran.lastName}` }}
            </td>
