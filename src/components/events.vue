  ...
  <v-card class="ma-2" min-width="350px">
    ...
    <v-card-text>
      <v-row v-for="(event, index) in events" :key="index">
        <v-checkbox multiple @change="saveVeteran()" readonly="event.name=='Reunion Registration'" :label="event.name" v-model="veteran.events" :value="event.name" class="my-1"></v-checkbox>
        <v-spacer />
        <span class="amt ma-2">${{ event.amt.toFixed(2) }}</span>
      </v-row>
      <v-row justify="end">
        <span class="amt ma-2 green--text total">Due for Events: </span>
        <div class="amt ma-2 green--text total text-right">${{ veteran.sum.toFixed(2) }}</div>
      </v-row>
    </v-card-text>
  </v-card>
</template>
  ...
  methods: {
    ...
    saveVeteran () {
      ...
      this.events.forEach(v => 
        sum += this.veteran.events.indexOf(v.name)>=0?v.amt:0
      )
